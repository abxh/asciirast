 ...
 */
using ProgramTokenGenerator = detail::StaticGenerator<ProgramToken, 512, 4>;
...
public:
    ...
    void deallocate(void* ptr)
    {
        ...
        assert(begin <= p && p < end && "pointer points to part of the pool");

        const auto offset = static_cast<std::uintptr_t>(p - begin);

        assert(offset % sizeof(Frame) == 0 && "pointer is aligned correctly");
...
public:
    ...
    struct Promise
    {
        ...
        StaticGenerator get_return_object() { return StaticGenerator{ handle_type::from_promise(*this) }; }
