...
lerp(const ProjectedFragment<T>& a, const ProjectedFragment<T>& b, const math::F t)
{
    ...
    const auto attrs = std::isfinite(a.z_inv) && std::isfinite(b.z_inv)
                               ? lerp_varying_perspective_corrected(a.attrs, b.attrs, t, a.z_inv, b.z_inv)
                               : lerp_varying(a.attrs, b.attrs, t);
